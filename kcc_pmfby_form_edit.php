...
$IdMD5 = $_GET['id'];
$page= $_GET['p'];
$query_application="SELECT * FROM `PMFBYKCC`  WHERE  `ID` = '$IdMD5'";
$result_application=mysql_query($query_application);
$num_application=mysql_numrows($result_application);
$BlockName = mysql_result($result_application,$i,"Block");
$FormSlNo = mysql_result($result_application,$i,"ApplicationNo");
$CID = mysql_result($result_application,$i,"CID");
$FID = mysql_result($result_application,$i,"FID");
$BeneficiaryName =  mysql_result($result_application,$i,"Name");
$SBEPIC =  mysql_result($result_application,$i,"UIDNO");
$MouGP=  mysql_result($result_application,$i,"GP");
...
$BankCodeDB =  mysql_result($result_application,$i,"BankCode");
$BankNameDB =  strtoupper(mysql_result($result_application,$i,"BankName"));
$BankBranchDB =  mysql_result($result_application,$i,"BankBranch");
$BranchCodeDB =  mysql_result($result_application,$i,"BranchCode");
...
$FathersName =  mysql_result($result_application,$i,"FatherName");
$AadharNo =  mysql_result($result_application,$i,"AadharNo");

$query_branch="select * from BankBranch WHERE TRIM(LEADING '0' FROM bank_cd )='$BankCodeDB' and TRIM(LEADING '0' FROM branchcd )='$BranchCodeDB'  ORDER BY branch_name ASC";
...
$BankBranchDB= $row_branch['branch_name']; } 
...
mysql_query("update PMFBYKCC set 
`KCCNo`='$fKCCNo',
`BankCode`='$fBankCode',
...
header ('Location: manage_pmfby_forms.php?msg=e10&p='.$page.'&blid='.$blkid.'&gp='.$GPMou);  
		} //F2
...
function bind_bank()
{
	...
	for (var i = 0; i < BankCode.options.length; i++) 
	{
		if (BankCode.options[i].value == "<?php echo $BankCd; ?>")
		{
			BankCode.options[i].selected = true;
		}
...
<body onload="javascript: bind_bank();">
...
<div id="hold">
  ...
  <tr>
    ...
    <td>Bank Name </td>
    <td><select name="BankName" id="BankName" class="required"  onChange="getBranch('findbankbranch.php?bank='+this.value)">
...
while($row_vol = mysql_fetch_assoc($result_vol))
{
  ...
  <option value="<? echo strtoupper($BankName); ?>" <? if(strtoupper($BankName)==$BankNameDB ){ echo 'Selected=Selected'; } ?> ><? echo $BankName; ?> </option>
...
<option value="<? echo $BankBranchDB ;?> "><? echo $BankBranchDB ; ?></option>
...
<? } ?>
    ...
    <td>IFSC:</td>
    <td> <input  type="hidden" name="IFSC" id="IFSC"> <div id="txtHint"><? echo $IFSC; ?></div></td>
  </tr>
  <tr>
    <td>Existing Loan:</td>
