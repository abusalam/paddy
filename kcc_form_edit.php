...
$BankNameDB =  mysql_result($result_application,$i,"BankName");
...
$AccountStatus =  mysql_result($result_application,$i,"AccountStatus");
$LoanRepaymentDate =  mysql_result($result_application,$i,"LoanRepaymentDate");
...
$fAccountStatus=  mysql_real_escape_string(stripslashes($_REQUEST['AccountStatus']));
$fLoanRepaymentDate = str_replace('/', '-', $fLoanRepaymentDate);
$fLoanRepaymentDate = date("Y-m-d", strtotime($fLoanRepaymentDate));
...
`AadharNo`='$fAadharNo',
`AccountStatus`='$fAccountStatus',
`LoanRepaymentDate`='$fLoanRepaymentDate' where ID='$IdMD5 '");
...
while($row_vol = mysql_fetch_assoc($result_vol))
{
  ...
  <option value="<? echo $BankCode; ?>" <? if($BankCode==$BankCodeDB ){ echo 'Selected=Selected'; } ?> ><? echo $BankName; ?> </option>
...
<option value="<? echo $BranchCodeDB;?> "><? echo $BankBranchDB; ?></option>
...
<? } ?>
  ...
  <tr>
   ...
   <td></td><td></td>
    ...
    <td><input type="text" size="10" pattern="\d{1,2}/\d{1,2}/\d{4}" placeholder="dd/mm/yyyy" name="LoanRepaymentDate" id="LoanRepaymentDate" value="<? echo $LoanRepaymentDate; ?>"></td>
  <td>Account Status (Dormant/Running):</td>
    <td><input type="radio" name="AccountStatus" value="Yes" <? if($AccountStattus=='Yes') { echo 'checked=checked'; }?> /> Yes | <input name="AccountStatus" type="radio" value="No" <? if($AccountStatus=='No') { echo 'checked=checked'; } ?> />No </td>
  </tr>
